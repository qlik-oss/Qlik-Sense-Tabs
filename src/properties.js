define(["qlik"], function (qlik) {
  ...
  function getMasterObjectList() {
    ...
    app.getAppObjectList('masterobject', function (data) {
      var sortedData = data.qAppObjectList.qItems.filter(function (item) {
        ...
        if (item1.qData.rank < item2.qData.rank) {
          return -1;
        }
        if (item1.qData.rank > item2.qData.rank) {
          return 1;
  ...
  var about = {
    ...
    items: {
      ...
      paragraph1: {
        label: `A container object that can contain several master visualizations. The user chooses
          which object to show by clicking on a tab.`,
        component: 'text'
      },
      paragraph2: {
